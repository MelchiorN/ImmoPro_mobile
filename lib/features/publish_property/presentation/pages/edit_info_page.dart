...
/// Page d'édition complète d'une annonce
class EditInfoPage extends StatefulWidget {
  final ListingEntity listing;

  const EditInfoPage({super.key, required this.listing});
  @override
...
class _EditInfoPageState extends State<EditInfoPage> {
  ...
  void initState() {
    ...
    // Initialiser les champs avec l'annonce existante
    _selectedType = widget.listing.typeBien;
    _selectedTransaction = widget.listing.typeTransaction;
    _titleCtrl.text = widget.listing.title;
    _priceCtrl.text = widget.listing.price.toStringAsFixed(0);
    _surfaceCtrl.text = widget.listing.surface?.toStringAsFixed(0) ?? '';
    _superficieCtrl.text = widget.listing.caracteristiques['superficie']?.toString() ?? '';
    _addressCtrl.text = widget.listing.location;
    _descCtrl.text = widget.listing.description ?? '';
    _descLength = _descCtrl.text.length;
    _roomsCtrl.text = widget.listing.rooms?.toString() ?? '1';
    _bathCtrl.text = widget.listing.bathrooms?.toString() ?? '1';

    _descCtrl.addListener(() =>
        setState(() => _descLength = _descCtrl.text.length));

    WidgetsBinding.instance.addPostFrameCallback((_) async {
      _controller.initForEdit(widget.listing);
      ...
      if (fetched.isNotEmpty && mounted) {
        setState(() {
          ...
          // Trouver le nom correspondant au slug
          final typeName = _typeToSlugMap.entries
              .where((e) => e.value == _selectedType)
              .map((e) => e.key)
              .firstOrNull;
          if (typeName != null) {
            _selectedType = typeName;
          } else if (!_propertyTypes.contains(_selectedType)) {
  ...
  Future<void> _onSave() async {
    ...
    await _controller.updateExistingProperty(widget.listing.id);

    if (!mounted) return;
    if (_controller.status == PublishStatus.success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Row(
            children: [
          ...
          backgroundColor: const Color(0xFF1A7B4B),
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          duration: const Duration(seconds: 3),
        ),
      );
      Navigator.of(context).pop(true); // true = annonce modifiée
        ...
        bottomNavigationBar: SafeArea(
          child: Padding(
            ...
            child: ListenableBuilder(
              listenable: _controller,
              builder: (_, __) {
                final isLoading = _controller.status == PublishStatus.loading;
                return SizedBox(
                  height: 52,
